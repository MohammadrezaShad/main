...
class Route {
  ...
  Auth = class Auth {
    ...
    static Login() {
      return new Uri(`${Auth.Base}/login`);
    }

    static Register() {
      return new Uri(`${Auth.Base}/register`);
    }

    static Password = class Password {
      static Base = `${Auth.Base}/password`;

      static Forget() {
        return new Uri(`${Password.Base}/forget`);
      }
    };
  };
}
